...
def v_employee_table():
    ...
    # Define the columns.
    tree["columns"] = ("Employee_ID", "Fname", "Lname", "Gender", "Date_Of_Birth", "Phone_Num", "Email", "Salary")
    ...
    tree.column("Salary", width = 150, minwidth = 150, anchor = CENTER)
    ...
    start = 0
    for row in myCursor:
        tree.insert("", start, text = "", values = (row[0], row[1], row[2], row[3], row[4], row[5], row[6],row[7]))
        start = start + 1
